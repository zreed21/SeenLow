...
export default function DealInboxPage() {
  ...
  const submit = async () => {
    ...
    setLinks(data.links || []);
    setUrls("");
    setMessage(`Saved ${data.added} URL(s). Timer ${data.endsAt ? "set" : "not parsed — use 01:22:25 or 4h"}.`);
  };
  ...
  const checkPrices = async (id?: number) => {
    ...
    setBusy(false);
    setMessage(data.success ? `Checked ${data.checked}` : (data.error || "Check failed"));
    load();
  };
  ...
  return (
    <div style={{ maxWidth: 880, margin: "0 auto", padding: "24px 16px", color: "#F0E6D8", minHeight: "100vh", background: "#0A0A0A" }}>
      ...
      <p style={{ color: "#A1A1AA", fontSize: 14, marginBottom: 20 }}>
        Save is inbox only. Publish to site creates the homepage card. Time left accepts 01:22:25 (h:m:s remaining), 4h, or 90m.
      </p>
      <textarea value={urls} onChange={(e) => setUrls(e.target.value)} placeholder="https://www.amazon.com/dp/B09YTXJM43" rows={5} style={{ width: "100%", background: "#121212", color: "#F0E6D8", border: "1px solid #262626", borderRadius: 12, padding: 12 }} />
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginTop: 10 }}>
        ...
        <input value={listedPrice} onChange={(e) => setListedPrice(e.target.value)} placeholder="List price 39.99" style={inputStyle} />
        <input value={salePrice} onChange={(e) => setSalePrice(e.target.value)} placeholder="Sale price 9.99" style={inputStyle} />
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10, marginTop: 12 }}>
        {live.map((row) => (
          <div key={row.id} style={{ background: "#121212", border: "1px solid #262626", borderRadius: 12, padding: 14 }}>
            <div style={{ fontWeight: 700 }}>{row.title || row.asin || row.domain || "Untitled"}{row.deal_id ? ` · live #${row.deal_id}` : ""}</div>
            <div style={{ fontSize: 12, color: "#A1A1AA", wordBreak: "break-all" }}>{row.url}</div>
            <div style={{ marginTop: 8, fontSize: 13 }}>Sale {row.sale_price ? `$${row.sale_price}` : "-"} · List {row.listed_price ? `$${row.listed_price}` : "-"} · {remaining(row.ends_at)}</div>
            <div style={{ marginTop: 8, display: "flex", gap: 10 }}>
              <a href={row.url} target="_blank" rel="noreferrer" style={{ color: "#B91C1C" }}>Open</a>
              <button onClick={() => publish(row.id)} style={{ background: "none", border: "none", color: "#34D399", cursor: "pointer" }}>Publish to site</button>
...
const inputStyle = { background: "#121212", color: "#F0E6D8", border: "1px solid #262626", borderRadius: 10, padding: "10px 12px" } as const;
